...
type StudentWithExtras = Student & {
  email: string;
  username: string;
  instagramUsernames: string; // Changed to string to hold comma-separated list
  totalViews: number;
  totalRevenue: number;
};
...
const TeacherListPage = async ({
  ...
  const columns = [
    ...
    { header: "Account", accessor: "account", className: "min-w-[200px]" }, // Increased width for multiple accounts
  ...
  // Process data to include total views, total revenue, and all Instagram usernames
  const data: StudentWithExtras[] = students.map((student) => {
    ...
    // Get all Instagram usernames as a comma-separated string
    const instagramUsernamesList = student.accounts.map((account) =>
      extractInstagramUsername(account.instagramLink)
    ).filter(Boolean); // Remove empty strings
    const instagramUsernames = instagramUsernamesList.length > 0 
      ? instagramUsernamesList.join(", ") 
      : student.email || "N/A"; // Fallback to email if no accounts
